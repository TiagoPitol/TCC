...
def FazAsImagens(timesteps, PotenciaSolar, PotenciaEolica, PotenciaH2, PotenciaBateria, CustoTotal, PotenciaTotal, previsoes, status_bateria, status_tanque):
    ...
    fig1.update_layout(title="Potências ao Longo do Tempo", xaxis_title="Etapas de Tempo", yaxis_title="Potência (kW)",
    ...
    fig3 = go.Figure()
    fig3.add_trace(
        go.Scatter(x=timesteps, y=PotenciaTotal, mode='lines+markers', name='Potência Total', line=dict(color='blue')))
...
def carregar_dados():
    ...
    if uploaded_file:
        ...
        if st.sidebar.button("Executar Previsão de Carga"):
            timesteps, PotenciaSolar, PotenciaEolica, PotenciaH2, PotenciaBateria, CustoTotal, PotenciaTotal, previsoes, status_bateria, status_tanque = executar_codigo(dados, preco_solar, preco_eolica, preco_h2, preco_bateria, dolar_hoje, Potenciadosistema, epochs,
                            batch_size, learning_rate, num_neurons, activation, passos_anteriores)
            FazAsImagens(timesteps, PotenciaSolar, PotenciaEolica, PotenciaH2, PotenciaBateria, CustoTotal, PotenciaTotal, previsoes, status_bateria, status_tanque)
