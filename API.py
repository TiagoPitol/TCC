...
carga_prevista = None
# Função para despacho econômico
def despacho_economico():
    ...
    global carga_prevista

    # Inicialização de parâmetros para cálculos
    Radiacao = np.array(geracao[18:306, 7])
    Temperatura = np.array(geracao[18:306, 6])
    Vento = np.array(geracao[18:306, 4])

    G_Solar = 0.97 * Radiacao / 1000 * (1 + 0.005 * (Temperatura - 25))
    ...
    Carga_Residencial = geracao[18:306, 12] * Carga
    Carga_Residencial = carga_prevista
    ...
    for i in range(len(potenciasolarmaxima_values)):
        ...
        limite_minimo_bateria = 0.1 * potenciaBateria_max

        if Carga_Residencial[i] <= potencia_disponivel:
        ...
        elif Carga_Residencial[i] <= potencia_disponivel + potenciaBateria_max:
            c = [preco_solar, preco_eolica, PrecoH2, preco_bateria]
            A_eq = [[1, 1, 0, 1]]
            b_eq = [Carga_Residencial[i]]
            bounds = [
                (0, potenciasolarmaxima),
            ...
            bounds = [
                ...
                (0, potenciaEolicamaxima),
                (0, potenciaH2maxima),
                (0, potenciaBateria_max),  # Bateria usada somente se acima de 10% de carga
            ]
...
def previsao_carga():
    ...
    if uploaded_file is None:
        st.warning("Por favor, carregue um arquivo Excel (CargaSul.xlsx) para continuar.")
        return
    ...
    carga = dados.iloc[:1000, 3].values  # Coluna 4 (índice 3)
    ...
    carga_prevista = model.predict(X_test)
    # % Avaliar o modelo
   ...
   # Título da aplicação
    ...
    return carga_prevista, previsoes
